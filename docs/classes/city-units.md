# Information technology - City data model - Part 1: Foundation level concepts - City Units Pattern

This ontology specifies the foundation-level concepts related to the city units pattern of the city data model.

This pattern imports the following files:

- [https://w3id.org/citydata/21972/v1/](https://w3id.org/citydata/21972/v1/)
- [https://w3id.org/citydata/part1/v1/Core](https://w3id.org/citydata/part1/v1/Core)

This pattern consists of the following classes:

- [Acceleration](Acceleration.md)
- [Acceleration Unit](AccelerationUnit.md)
- [Amount Of Money](AmountOfMoney.md)
- [Area](Area.md)
- [Area Unit](AreaUnit.md)
- [Capacity](Capacity.md)
- [Capacity Size](CapacitySize.md)
- [Cardinality Measure](CardinalityMeasure.md)
- [Cardinality Unit Per Time](CardinalityUnitPerTime.md)
- [City Units Thing](CityUnitsThing.md)
- [Duration](Duration.md)
- [Geo_position_unit](Geo_Position_unit.md)
- [Length](Length.md)
- [Length Unit](LengthUnit.md)
- [Mass](Mass.md)
- [Mass Unit](MassUnit.md)
- [Monetary Value](MonetaryValue.md)
- [Ratio](Ratio.md)
- [Speed](Speed.md)
- [Speed Unit](SpeedUnit.md)
- [Time Unit](TimeUnit.md)
- [Value Of Money](ValueOfMoney.md)
- [Volume](Volume.md)
- [Volume Unit](VolumeUnit.md)
This module defines the following properties:

- [CityUnitObjectProperty](../properties/CityUnitObjectProperty.md)
- [CityUnitsDataProperty](../properties/CityUnitsDataProperty.md)
- [hasRelativeYear](../properties/hasRelativeYear.md)


The formal definition of this pattern is available in TURTLE Syntax in two files, the [core semantics](../city-units-pattern.ttl) and the SHACL [restrictions](../city-units-shacl.ttl).
